...
# DBA fragment and read densities
Note: According to the `ChIPsim` vignette, the function `ChIPsim::bindDens2readDens` generates read densities for the forward and reverse strand. I find this description ambiguous, as the function really simulates *DNA fragment densities* from both strands. *Reads* are simulated in a subsequent step, using the function `ChIPsim::sampleReads`. To maintain consistency with the vignette description, I will continue to refer to "read densities" in the context of `ChIPsim::bindDens2readDens`. 

## DNA fragment size distribution 

We first define a DNA fragment size distribution function. Here we estimate the standard deviation of the length distribution from the range
\begin{equation}
...
We can now generate read densities (better: DNA fragment densities) for the forward and reverse strand using the binding site densities `dens` and DNA fragment length distribution function `fragLength`. 
```{r}
